function eyeRecons(F, start, final, base_folder, image_folder, plyfile, step, stereoParams)
...
for i = start:final
    ...
    right_fn = sprintf('%s%s/right%d.tiff',base_folder, image_folder,  i);
    left_fn = sprintf('%s%s/left%d.tiff',base_folder, image_folder,  i);
    right = imread(right_fn);
    right_img = rgb2gray(right);
    right_img_bw = im2bw(im2double(right_img), threshR);
    
    right_img = uint8(right_img_bw).*right_img;
    
    % For left camera image
    left = imread(left_fn);
    colorImgL = colorImgL + left;
    left_img = rgb2gray(left);
    left_img_bw = im2bw(im2double(left_img), threshL);
    left_img = uint8(left_img_bw).*left_img;
